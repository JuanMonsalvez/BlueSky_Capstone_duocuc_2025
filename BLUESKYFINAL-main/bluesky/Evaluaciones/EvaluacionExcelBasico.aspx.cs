...
{
    public partial class EvaluacionExcelBasico : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            // === VALIDAR SESIÓN SEGÚN TU LOGIN ===
            string usuarioId = Convert.ToString(Session["USUARIO_ID"]);

            // Si no hay USUARIO_ID, no hay login válido → ir a IniciarSesion
            ...
            // 1) Validar máximo 3 intentos usando el RUT guardado en USUARIO_ID
            ...
            // 2) Cargar preguntas + alternativas (AQUÍ VA)
        ...
        private bool TieneMaxIntentos(string rut, int cursoId)
        {
            ...
            using (var cmd = new MySqlCommand(
                "SELECT COUNT(*) FROM intento_evaluacion WHERE persrut=@rut AND curso_id=@curso", cn))
            {
        ...
        private void CargarPreguntasYAlternativas(int cursoId)
        {
            ...
            // Guardamos todo en ViewState para corregir luego sin consultar BD
            ...
            // Tomar solo 15 por si el día de mañana agregas más
        ...
        // =======================
        //  NUEVO: DESCARGAR CERTIFICADO (iText7)
        // =======================
        protected void btnCertificado_Click(object sender, EventArgs e)
        {
            ...
            using (var ms = new MemoryStream())
            {
                ...
                // Colores más marcados
                Color colorBorde = new DeviceRgb(13, 110, 253);   // azul
                Color colorTexto = new DeviceRgb(33, 37, 41);     // casi negro
                Color colorDetalle = new DeviceRgb(108, 117, 125);  // gris para detalles

                // Crear página y documento
                ...
                if (File.Exists(rutaMarcaAgua))
                {
                    ...
                    // Dibujar imagen semitransparente
                    ...
                    // 👇 ESTA ES LA LÍNEA CORRECTA EN ITEXT 7
                    canvasWatermark.AddImageWithTransformationMatrix(
                        marcaAguaData,
                        marcaAncho, 0,                 // escala en X
                        0, marcaAlto,                  // escala en Y
                        centerX - (marcaAncho / 2),    // traslación X
                        centerY - (marcaAlto / 2)      // traslación Y
                    );
        ...
        private void GuardarRespuestas(int intentoId)
        {
            DataTable dtAll = ViewState["dtAll"] as DataTable;
                ...
                using (var tx = cn.BeginTransaction())
                {
                    try
                    {
                        foreach (RepeaterItem item in rptPreguntas.Items)
                        {
                          ...
                          VALUES (@intento, @pregunta, @alternativa);", cn, tx))
                            {
                                cmdIns.Parameters.AddWithValue("@intento", intentoId);
                                cmdIns.Parameters.AddWithValue("@pregunta", preguntaId);
                                cmdIns.Parameters.AddWithValue("@alternativa", alternativaId);
                                cmdIns.ExecuteNonQuery();
                            }
