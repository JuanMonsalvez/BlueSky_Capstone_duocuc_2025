...
namespace bluesky.Evaluaciones
{
    public partial class EvaluacionCiberseguridadBasica : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            int cursoId = 4; // Ciberseguridad Básica (ajusta si en tu BD es otro ID)
        ...
        protected void btnFinalizar_Click(object sender, EventArgs e)
        {
            ...
            // 1) Calcular correctas como ya lo hacías
            ...
            // 2) Insertar intento + obtener intento_id
            using (var cn = new MySqlConnection(cs))
            {
                cn.Open();
                      ...
                      VALUES (@rut, @curso, NOW(), @puntaje, @numc);", cn, tx))
                        {
                            ...
                            // MySqlCommand tiene la propiedad LastInsertedId
                            intentoIdGenerado = (int)cmd.LastInsertedId;
                        }

                        // 3) Guardar respuestas individuales
                        foreach (RepeaterItem item in rptPreguntas.Items)
                        {
            ...
            // 4) Mostrar resultado (igual que antes)
            pnlEvaluacion.Visible = false;
        ...
        protected void btnCertificado_Click(object sender, EventArgs e)
        {
            ...
            string nombreCurso = "Ciberseguridad Básica";
            ...
            using (var ms = new MemoryStream())
            {
                ...
                Response.AddHeader("content-disposition", "attachment; filename=Certificado_CiberseguridadBasica.pdf");
        ...
        private void GuardarRespuestas(int intentoId, DataTable dtAll)
        {
            using (var cn = new MySqlConnection(cs))
            {
                cn.Open();
                ...
                using (var tx = cn.BeginTransaction())
                {
                    try
                    {
                        foreach (RepeaterItem item in rptPreguntas.Items)
                        {
                            ...
                            // Si no respondió esta pregunta, no se guarda nada
                            if (hfPreguntaId == null || rblAlt == null ||
                                string.IsNullOrEmpty(rblAlt.SelectedValue))
                                continue;
